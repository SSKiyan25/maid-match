...
import { MaidData, UserPhoto } from "@/types";
import { Image, Upload, Trash2, Pencil, Star, RefreshCw } from "lucide-react";
import {
    Dialog,
    DialogContent,
    DialogHeader,
...
export default function PhotosTab({ maid }: PhotosTabProps) {
    ...
    const {
        ...
        initialPhotos: maid.maid.user.photos || [],
    ...
    const refreshPhotos = async () => {
        try {
            const response = await axios.get(
                `/user-photos?user_id=${maid.maid.user.id}`
            );
    ...
    return (
        <div className="p-6 pt-5 space-y-6">
            <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-semibold">Maid Photos</h2>
                <Button
                    size="sm"
                    variant="outline"
                ...
                <div className="border rounded-lg p-4 bg-muted/30">
                    {bannerPhoto ? (
                        <div
                            ...
                            onClick={() => togglePhotoActions(bannerPhoto.id)}
                        >
                            <img
                                src={getImageUrl(bannerPhoto.url)}
                                alt={bannerPhoto.caption || "Profile Banner"}
                                ...
                                <div className="absolute inset-0 bg-black/50 flex items-center justify-center rounded-md">
                                    <div className="flex gap-2">
                                        ...
                                        <Button
                                            ...
                                            variant="destructive"
                                            onClick={(e) =>
                                                confirmDelete(bannerPhoto.id, e)
                                            }
                                        >
                        ...
                        <div className="flex flex-col items-center justify-center h-[200px] border-2 border-dashed rounded-md p-4 text-center">
                            ...
                            <Button
                                size="sm"
                                variant="secondary"
                                onClick={() => handleOpenAddPhotoDialog(true)}
                            >
                ...
                <p className="text-xs text-muted-foreground mt-3">
                    ...
                    {galleryPhotoCount >= 6 && (
                        <span className="text-amber-600 ml-1">
                            You've reached the maximum of 6 photos. Delete some
                            to add more.
                        </span>
                    )}
                </p>
